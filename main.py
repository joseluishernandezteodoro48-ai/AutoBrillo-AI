...
api_limiter = RateLimiter(
    limit=int(os.getenv("AUTOBRILLO_RATE_LIMIT", "60")), window_seconds=60
)
...
def require_admin(x_admin_key: str | None = Header(default=None)):
    ...
    if not constant_time_equal(x_admin_key, expected):
...
def require_webhook_key(x_autobrillo_webhook_key: str | None = Header(default=None)):
    ...
    if not expected:
        raise HTTPException(503, "Webhook seguro no configurado.")
    if not constant_time_equal(x_autobrillo_webhook_key, expected):
        raise HTTPException(401, "Webhook no autorizado.")


def persistence_status():
    db = oauth.database_health()
    mem = agent.memory.health()
    return {
        "postgres_configured": db["configured"] or mem.get("configured", False),
        "postgres_connected": db["connected"] or (mem.get("connected") is True),
        "postgres_error": db.get("error") or mem.get("error"),
        "token_storage": "postgresql" if db.get("connected") else "local_fallback",
        "business_storage": mem.get("backend", "sqlite"),
        "business_connected": mem.get("connected") if mem.get("backend") == "postgresql" else True,
    }
...
<title>Brillo · AutoBrillo AI</title>
<style>
body{margin:0;font-family:system-ui;background:#0b1020;color:#f4f7ff}
main{max-width:900px;margin:auto;padding:28px}
.card{background:#151d32;border:1px solid #293451;border-radius:18px;padding:20px;margin:14px 0}
h1{margin-bottom:4px}small{color:#aab5cf}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}
button{width:100%;padding:16px;border:0;border-radius:12px;background:#263454;color:white;font-size:16px;cursor:pointer}
...
Las publicaciones, compras, cobros y transferencias siguen bloqueadas hasta contar
con conectores y permisos oficiales (dry-run activo por defecto).</small></p>
</div>
<div class="card"><h2>Resultado</h2>
<pre id="out">Brillo está listo. Escribe una orden.</pre></div>
</main>
<script>
...
def run_autonomy(request: AutonomyRequest):
    try:
        ...
        raise HTTPException(
            400, f"Tygo no pudo ejecutar autonomía: {redact(exc)}"
        ) from None
...
def mercadolibre_me():
    try:
        ...
        raise HTTPException(
            400, f"No se pudo consultar Mercado Libre: {redact(exc)}"
        ) from None
...
def mercadolibre_search(q: str, limit: int = 5):
    ...
    try:
        return ml_api.search(q, limit)
    except Exception as exc:
        raise HTTPException(
            400, f"No se pudo buscar en Mercado Libre: {redact(exc)}"
        ) from None


@app.get("/api/mercadolibre/item/{item_id}", dependencies=[Depends(rate_limit)])
def mercadolibre_item(item_id: str):
    if len(item_id) > 100 or any(ch in item_id for ch in '<>"\''):
        raise HTTPException(400, "Identificador de producto inválido.")
    try:
        return ml_api.item(item_id)
    except Exception as exc:
        raise HTTPException(
            400, f"No se pudo consultar el producto: {redact(exc)}"
        ) from None
...
def mercadolibre_orders(offset: int = 0, limit: int = 20):
    if not 1 <= limit <= 50:
        raise HTTPException(400, "limit debe estar entre 1 y 50.")
    try:
        return ml_api.orders(offset=offset, limit=limit)
    except Exception as exc:
        raise HTTPException(
            400, f"No se pudieron obtener órdenes: {redact(exc)}"
        ) from None


@app.get(
    "/api/mercadolibre/orders/{order_id}",
    dependencies=[Depends(require_admin), Depends(rate_limit)],
)
def mercadolibre_order(order_id: str):
    try:
        return ml_api.order(order_id)
    except Exception as exc:
        raise HTTPException(
            400, f"No se pudo obtener la orden: {redact(exc)}"
        ) from None
...
def brillo_command(request: BrilloRequest):
    try:
        ...
        if (
            intent.get("intent") in ("source", "source_and_sell")
            and intent.get("product_count_source") != "user"
        ):
            return autonomy.run(request.command, None)
        return brillo.respond(request.command)
    except Exception as exc:
        raise HTTPException(
            400, f"Brillo no pudo procesar la orden: {redact(exc)}"
        ) from None
...
def plan(limit: int | None = None):
    ...
    count = (
        ...
        else int(brillo.understand("productos")["product_count"] or 10)
    )
    return {
        "tygo_decision": brillo.understand("productos"),
        "tasks": agent.plan(count),
    }
...
def cycle(limit: int | None = None):
    ...
    return {
        "tygo_decision": brillo.understand("productos"),
        "cycle": agent.run_cycle(count, ml_api=ml_api),
    }
...
def prepare_listing(request: PublishRequest):
    """Arma el payload de publicación sin enviarlo a ML (siempre seguro)."""
    try:
        return agent.prepare_listing(request.product_name, category_id=request.category_id)
    except Exception as exc:
        raise HTTPException(400, f"No se pudo preparar listing: {redact(exc)}") from None
...
def publish_listing(request: PublishRequest):
    """Publica en Mercado Libre.

    Requiere:
      - AUTOBRILLO_DRY_RUN=false
      - permiso sales habilitado
      - OAuth conectado
      - category_id válido (o ML_DEFAULT_CATEGORY_ID)
    """
    try:
        return agent.publish_product(
        ...
        raise HTTPException(403, redact(exc)) from None
    except Exception as exc:
        raise HTTPException(400, f"No se pudo publicar: {redact(exc)}") from None
...
def create_goal(request: GoalRequest):
    try:
        ...
        raise HTTPException(400, redact(exc))
...
def oauth_callback(code: str | None = None, state: str | None = None):
    ...
    try:
        ...
        raise HTTPException(
            400, f"No se pudo completar OAuth: {redact(exc)}"
        ) from None
    return (
        "AutoBrillo AI: Mercado Libre conectado correctamente. "
...
@app.post(
    ...
    status_code=200,
    dependencies=[Depends(require_webhook_key), Depends(rate_limit)],
)
async def mercadolibre_webhook(payload: dict):
    """Recibe notificaciones de ML. Guarda el evento y, si es una orden, la registra."""
    topic = str(payload.get("topic") or payload.get("resource") or "unknown")
    ...
    # Si llega un ID de orden, intentar enriquecer (solo lectura, dry-run seguro)
    if "orders" in topic.lower() or "/orders/" in resource:
        order_id = resource.rstrip("/").split("/")[-1] if resource else None
        if order_id and order_id.isdigit():
            try:
                order_data = ml_api.order(order_id)
                agent.memory.remember(
                    "order_fetched",
                    {
                        "order_id": order_id,
                        "status": order_data.get("status"),
                        "items": len(order_data.get("order_items") or []),
                    },
                    "fetched",
                )
            except Exception as exc:
                agent.memory.remember(
                    "order_fetch_failed",
                    {"order_id": order_id, "error": redact(exc)},
                    "error",
                )
    return {"ok": True}
